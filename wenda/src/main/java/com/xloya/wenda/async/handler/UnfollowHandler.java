...
public class UnfollowHandler implements EventHandler {
    ...
    public void doHandle(EventModel eventModel) {
        ...
        if(eventModel.getEntity_type()== EntityType.ENTITY_QUESTION)
            message.setContent("用户"+user.getName()+"取消关注了你的问题，http://localhost:8080/question/"+eventModel.getEntity_id());
        else if(eventModel.getEntity_type()== EntityType.ENTITY_USER)
            message.setContent("用户"+user.getName()+"取消关注了你，http://localhost:8080/user/"+eventModel.getActor_id());
